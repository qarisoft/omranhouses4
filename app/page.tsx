...
const data2: PageItem[] = [
  { className: "", index: 0, imgUrl: 'alnakheel/out/1.jpg', mobileImgUrl: 'alnakheel/out/1.jpg', title: 'مشروع منازل 102', address: 'حي النخيل' },
  { className: "", index: 1, imgUrl: 'morowa/out/1.jpg', mobileImgUrl: 'morowa/out/1.m.jpg', title: 'مشروع منازل المروة', address: 'حي المروة' },
  { className: "", index: 2, imgUrl: 'morowa/out/2.jpg', mobileImgUrl: 'morowa/out/2.m.jpg', title: 'مشروع منازل المروة', address: 'حي المروة' },
  { className: "", index: 3, imgUrl: 'morowa/out/3.jpg', mobileImgUrl: 'morowa/out/3.m.jpg', title: 'مشروع منازل المروة', address: 'حي المروة' },
...
function HeroSection() {
  ...
  const previous = activeIndex > 0 ? data[activeIndex - 1] : data[data.length - 1]
  const doShift = () => {
    const d = data
  ...
  return (
    <AnimatePresence >
        ...
        {data.map((item, index) => {
          return (
            <motion.div
              ...
              className={"absolute top-0 left-0 bg-cover bg-no-repeat bg-black shrink-0  h-[calc(100vh)] w-screen animate-img an   "
      ...
      {active && (
        ...
        <div className="absolute top-1/2 left-1/2 z-10 .animate__animated animate__fadeInDown" key={active.index + 'title'}>

          <div className="p-5 bg-[#00000033]">
            <div className="text-gray-100 shadow-sm text-4xl">{active.title}</div>
            <div className="text-gray-100 shadow-sm text-2xl">{active.address}</div>
          </div>
        </div>
      )}
    </AnimatePresence>
